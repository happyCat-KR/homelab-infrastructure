...
HomeLab 환경을 구축하기 위해 물리 서버에 Ubuntu Server를 설치하고
원격 관리 및 가상화 환경을 구성하였다.

### OS Installation
- Rufus를 이용하여 Ubuntu Server ISO 부팅 USB 생성
- BIOS 설정을 통해 USB 부팅 구성
- Ubuntu Server 24.04 설치

### Server Configuration
- SSH 서버 설치 및 원격 접속 환경 구성
- SSH Key 기반 인증 구성
- Password 로그인 비활성화
- UFW 방화벽 설정
...
- Home Server IP 고정
- Router 포트포워딩을 통한 외부 SSH 접근 설정

### Virtualization Environment
- KVM / libvirt 설치
- Bridge Network 구성 (enp2s0 → br0)
- VM이 Router DHCP에서 직접 IP를 할당받도록 설정

### VM Network Management
- VM MAC 주소 기반 DHCP 예약 설정
- 재부팅 이후에도 VM IP가 유지되도록 구성

### Container Runtime
- Docker 설치
- Docker Compose 설치
- VM별 컨테이너 기반 서비스 배포 환경 구성

## 4. Virtualization Layout
...
## 7. Service Deployment Structure
...
## 11. Large Dataset Performance Test (100k rows)
